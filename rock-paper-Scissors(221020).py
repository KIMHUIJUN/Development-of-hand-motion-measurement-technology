...
#     if not ret:
#         continue
img = cv2.imread("KakaoTalk_20221018_171406355_02.jpg")
...
if result.multi_hand_world_landmarks is not None:
    rps_result = []
    a = 0
    for res in result.multi_hand_world_landmarks:
        joint = np.zeros((21, 3))
        ...
        print("r_np.einsum('nt,nt->n',v[[0,1,2,4,5,6,8,9,10,12,13,14,16,17,18],:], v[[1,2,3,5,6,7,9,10,11,13,14,15,17,18,19],:]",np.einsum('nt,nt->n',
            v[[0,1,2,4,5,6,8,9,10,12,13,14,16,17,18],:], 
        ...
        print("degress_angle = np.degrees",angle)
        
        # Inference gesture
        data = np.array([angle], dtype=np.float32)
        print("data = np.array([angle], dtype=np.float32) ",data)
        ret, results, neighbours, dist = knn.findNearest(data, 3)
        print("result = ret, results, neighbours, dist = knn.findNearest(data, 3) ", result)
        idx = int(results[0][0])
        print("idx = int(results[0][0])" , idx)
        # Draw gesture result
        ...
        mp_drawing.plot_landmarks(res, mp_hands.HAND_CONNECTIONS, azimuth = 5)
